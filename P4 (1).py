...
# ---
# 
# * Estudiante: **Deiler Jiménez Silva**
# * Carné: **B53700**
# * Grupo: **2**
# 
# ---
...
# 4.1. - Modulación 16-QAM
# En este apartado se Realizó una simulación del sistema de comunicaciones como en la sección 3.2., 
# pero utilizando una modulación 
# 16-QAM en lugar de una modulación BPSK. Deben mostrarse las imágenes enviadas y recuperadas y 
# las formas de onda.
...
def modulador(bits, fc, mpp):
    ...
    # 3. Inicializar la señal modulada
    t_simulacion = np.linspace(0, N*Tc, N*mpp) 
    senal_Tx = np.zeros(t_simulacion.shape)
    ...
    # 5. Calculo de la potencia promedio de la señal modulada
    Pm = (1 / (N*Tc)) * np.trapz(pow(senal_Tx, 2), t_simulacion)
    
    sumatoria = portadora1 + portadora2
    
    return senal_Tx, Pm, sumatoria, moduladora  
    
    
def demodulador(senal_Rx, sumatoria, mpp):
    '''Un método que simula un bloque demodulador
    de señales, bajo un esquema BPSK. El criterio
    de demodulación se basa en decodificación por 
    detección de energía.
    ...
    :return: Los bits de la señal demodulada
    '''
    # número de muestras en senal_Rx
    M = len(senal_Rx)

    # número de bits en transmisión
    N = int(M / mpp)
    ...
    Es = np.sum(sumatoria**2)

    # Aquí comienza la modulación
    for i in range(N):
        
        producto = senal_Rx[i*mpp : (i+1)*mpp] * sumatoria
        senal_demodulada[i*mpp : (i+1)*mpp] = producto
        Ep = np.sum(producto) 
...
#Vemos como el parámetro SNR nos mejora la imagen 
...
# 3. Modular la cadena de bits usando el esquema BPSK
senal_Tx, Pm, sumatoria, moduladora = modulador(bits_Tx, fc, mpp)

# 4. Se transmite la señal modulada, por un canal ruidoso
senal_Rx = canal_ruidoso(senal_Tx, Pm, SNR)

# 5. Se desmodula la señal recibida del canal
bits_Rx, senal_demodulada = demodulador(senal_Rx, sumatoria, mpp)
...
# La onda cuadrada moduladora (bits de entrada)
ax1.plot(moduladora[0:500], color='r', lw=2) 
ax1.set_ylabel('$b(t)$')
...
# La señal modulada al dejar el canal
ax3.plot(senal_Rx[0:500], color='b', lw=2) 
ax3.set_ylabel('$s(t) + n(t)$')

# La señal demodulada
ax4.plot(senal_demodulada[0:500], color='m', lw=2) 
ax4.set_ylabel('$b^{\prime}(t)$')
ax4.set_xlabel('$t$ / milisegundos')
fig.tight_layout()
plt.show()
...
# para obtener conclusiones respecto a esto.
